...
int get_count(int n, int lg, int cur) {
    int val = (n >> (lg+1));
    if (n&(1<<lg) && (n%(1<<lg) >= cur)) {
        val += 1;
    }
    return val;
}
...
void solve(){
    ...
    vector<int> cur;
    vector<int> res_list;
    frep(i,1,n) cur.push_back(i);
    int ans = 0;
    for (int lg = 0; lg<LG; lg++) {
        int target_cnt = get_count(n,lg,ans);
        // dbgl(target_cnt);
        int cur_cnt = 0;
        for (auto i:cur) {
            ask(i,(1<<lg));
            int res; cin >> res; cur_cnt += res;
            res_list.push_back(res);
        }
        if (cur_cnt != target_cnt) {
            ans += (1<<lg);
            for (int i = 0; i < cur.size(); i++){
                if (res_list[i]) nxt.push_back(cur[i]);
            }
        }
