...
int getprefpts(int idx) {
    if (idx == -1) return 1;
    else return pref[idx];
}
...
int getpref(int l, int r) {
    if (l == -1) {
        ...
        return (getprefpts(r) - getprefpts(l-1) + MD)%MD;
    }
}

void updatepref(int idx) {
    pref[idx] = (dp[idx] + getprefpts(idx-1))%MD;
}
...
void solve(){
    ...
    priority_queue<pii,vector<pii>,greater<pii>> pqcur;
    ffor(i,1,n) {
        cin >> a[i];
        for (int idx = 0; idx < a[i]; idx++) {
            pq.push({idx*i, (idx+1)*i-1});
        }
        int l = min(n,i*(a[i])), r = min(n,i*(a[i]+1));
        pref[l]++;
        pref[r]--;
    }
    int cur = 0;
    ...
    for(int i = 0; i < n; i++){
        int cnt = 0;
        while (!pqcur.empty() && pqcur.top().fi < i) {
            mx_prev = max(mx_prev, pqcur.top().se);
            --cur;
            pqcur.pop();
        }
        ...
        if (!block[i]) {
            dp[i] = (dp[i] + getpref(mx_prev, i-1))%MD;
        }
        updatepref(i);
    }

    while (!pqcur.empty() && pqcur.top().fi < INF) {
        mx_prev = max(mx_prev, pqcur.top().se);
        --cur;
        pqcur.pop();
    }
