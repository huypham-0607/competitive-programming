...
void solve(){
    ...
    ffor(i,1,n) {
        ...
        if (a[i] != 0) {
            if (a[i-1] == 0) {
                frep(x,0,MX) {
                    dp[i][x] = min(dp[i][x], max(dp[i-1][0],x+1));
                }
            }
            else if (a[i-1] != a[i]) {
                frep(x,0,MX) {
                    frep(y,0,MX) {
                        dp[i][x] = min(dp[i][x], max(dp[i-1][y],x+y+2));
                    }
                }
            }
            else {
                frep(x,0,MX) {
                    frep(y,0,MX) {
                        if (x==y) continue;
                        dp[i][x] = min(dp[i][x], max(dp[i-1][y],abs(x-y)));
                    }
                }
            }
        }
            ...
            frep(y,0,MX) {
                dp[i][0] = min(dp[i][0], max(dp[i-1][y], y+1)); 
            }
        }
    }
